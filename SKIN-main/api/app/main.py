...
def _ensure_analyses_model_used_column():
    try:
        insp = inspect(engine)
        ...
        with engine.begin() as conn:
            conn.execute(text("ALTER TABLE analyses ADD COLUMN model_used VARCHAR(128)"))
    except Exception as e:
        print("Schema migration model_used:", e)
...
async def lifespan(app: FastAPI):
    ...
    _ensure_analyses_model_used_column()
